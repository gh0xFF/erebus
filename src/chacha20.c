...
#include "chacha20.h"
    ...
    #define CHACHA20_SIMD_NEON 0
    #define CHACHA20_SIMD_AVX2 0
#elif defined(__AVX2__)
    #define CHACHA20_SIMD_AVX2 1
    #define CHACHA20_SIMD_NEON 0
...
void chacha20_xor(struct chacha20_context *ctx, uint8_t *bytes, size_t n_bytes) {
    ...
    for (size_t i = 0; i < n_bytes; i += 32) {
        ...
        __m256i key = _mm256_loadu_si256((__m256i*)(keystream8 + ctx->position));
        __m256i res = _mm256_xor_si256(data, key);
        _mm256_storeu_si256((__m256i*)(bytes + i), res);
        ctx->position += 32;
    }

    #elif CHACHA20_SIMD_NEON
    ...
    for (size_t i = 0; i < n_bytes; i += 16) {
        ...
        uint8x16_t data = vld1q_u8(bytes + i);
        uint8x16_t key = vld1q_u8(keystream8 + ctx->position);
        uint8x16_t res = veorq_u8(data, key);
        vst1q_u8(bytes + i, res);
        ctx->position += 16;
    }
